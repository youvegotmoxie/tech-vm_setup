...
sub append_vms_log {
    ...
    open( my $fh, ">>", $VMS_LOG )
      or return print_warn("Unable to open $VMS_LOG: $!");
    print $fh $line;
    close $fh;
...
sub disable_ea4_experimental {
    if ( -e '/etc/yum.repos.d/EA4-experimental.repo' ) {
        ...
        open( my $read, '<', '/etc/yum.repos.d/EA4-experimental.repo' )
          or return print_warn("Unable to open /etc/yum.repos.d/EA4-experimental.repo: $!");
        open( my $write, '>', '/etc/yum.repos.d/EA4-experimental.repo.vmstmp' )
          or return print_warn("Unable to open /etc/yum.repos.d/EA4-experimental.repo.vmstmp: $!");
        ...
        rename( '/etc/yum.repos.d/EA4-experimental.repo.vmstmp', '/etc/yum.repos.d/EA4-experimental.repo' )
          or return print_warn("Unable to rename /etc/yum.repos.d/EA4-experimental.repo.vmstmp: $!");
    }
    return;
}
